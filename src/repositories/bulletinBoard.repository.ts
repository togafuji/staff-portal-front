...
import type {
  ...
  UpdateBulletinBoardPayload
} from '@/entity/bulletinBoard.entity'
import { BulletinBoardConstants } from '@/constants/bulletinBoard.constants'

// データマッピング関数
const mapToBulletinBoardItem = (data: any): BulletinBoardItem => {
  return {
    id: data.id,
    title: data.title,
...
export const BulletinBoardRepository = {
  // 全掲示板投稿を取得
  async fetchBulletinBoards(): Promise<BulletinBoardItem[]> {
  ...
  // 特定の掲示板投稿を取得
  async fetchBulletinDetail(id: number): Promise<BulletinBoardItem> {
    const response = await axios.get(`${BulletinBoardConstants.BULLETIN_BOARDS}/${id}`)
    return mapToBulletinBoardItem(response.data)
  },

  // 新規掲示板投稿を作成
  async createBulletin(payload: CreateBulletinBoardPayload): Promise<BulletinBoardItem> {
    try {
  ...
  async updateBulletin(
    id: number,
    payload: UpdateBulletinBoardPayload
  ): Promise<BulletinBoardItem> {
    try {
